...
const updateSizeWithMessage = (element, scalesPageToFit) =>
  ...
  function updateSize() {
    ...
    window.ReactNativeWebView.postMessage(JSON.stringify({ width: Math.min(width, screen.width), height: height , topic: ${topicString} }));
...
const getScript = ({files, customStyle, customScript, style, viewportContent, scalesPageToFit, scrollEnabledWithZoomedin}) => {
