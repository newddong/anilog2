...
import {Bracket, DownBracketBlack, ProgressbarAssign_1} from 'Asset/image';
...
export default FirtStage = props => {
   ...
   const goNext =()=>{
      ...
      props.navigation.push('Assign_shelter_step2',{title: '보호소 등록',data:data});
   }
	...
	return (
		<View style={lo.wrp_main}>
			<View style={lo.contents}>
				...
				<SvgWrapper style={{width: 650 * DP, height: 56 * DP, marginBottom: 78 * DP}} svg={<ProgressbarAssign_1/>} />
				<View style={[lo.shelter_form, {marginTop: 70 * DP}]}>
					...
					<View style={{flexDirection: 'row', alignItems: 'center', marginBottom: 10 * DP}}>
                     ...
                     value={data.shelter_addr}
						...
						<TouchableWithoutFeedback
							onPress={() => {
								navigation.navigate('AddressSearch', {title: '주소검색', from: 'Assign_shelter_step1', addr:data.shelter_addr});
								// alert('주소검색');
							}}>
							<View style={[btn.search_address, btn.shadow]}>
								<Text style={[txt.noto28b, txt.white]}>주소검색</Text>
							</View>
						</TouchableWithoutFeedback>
					</View>
					<FormTxtInput
